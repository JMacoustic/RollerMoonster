...
from utils import centerNvector
...
class Cart:
    ...
    def create_cart(self):
        self.cart = geometry.Cube(width= 0.2, height=0.1, depth= 0.4, batch=self.cartBatch, color=self.color)
    ...
    def move(self, u):
        ...
        if self.frame == "frenet_frame":
            x, y, z = self.spline.frenet_frame(u)
        self.cart.matrix = Mat4.from_translation(0.03*Vec3(*y)) @ centerNvector(Mat4, self.spline.coordinate(u), z, y)